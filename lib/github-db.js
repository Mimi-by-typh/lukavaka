...
module.exports = {
    ...
    async getOnlineSessions() {
        const data = await getData();
        return data.onlineSessions || {};
    },

    async saveOnlineSessions(sessions) {
        const data = await getData();
        data.onlineSessions = sessions;
        return saveData(data);
    },

    async updateOnlineSession(sessionId, userId, timestamp) {
        const data = await getData();
        data.onlineSessions = data.onlineSessions || {};
        data.onlineSessions[sessionId] = { userId, lastActivity: timestamp };
        return saveData(data);
    },
